...
def usage(s)
    ...
    puts "Usage: adaptation-eld <show|set> <him|all> [value]"
        ...
        if t.id == :CaveAdapt
            ...
            if mode == 'show'
                if df.respond_to?(:print_color)
                    ...
                    case t.value
                    when 0..399999
                        df.print_color(COLOR_GREEN, "Unit #{u.id} (#{u.name}) has an adaptation of #{t.value}\n")
                        #print "#{t.value}\n"
                    when 400000..599999
                        df.print_color(COLOR_YELLOW, "Unit #{u.id} (#{u.name}) has an adaptation of #{t.value}\n")
                    else
